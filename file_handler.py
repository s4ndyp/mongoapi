...
def upload_file(endpoint):
    ...
    if file:
        ...
        # Gebruik endpoint en client_id voor de mappenstructuur
        # local_storage/<endpoint>/<client_id>/<filename>
        target_dir = os.path.join(UPLOAD_FOLDER, endpoint, client_id)
        ...
        try:
            ...
            # We sturen de client_id in de URL-parameters mee voor de GET-aanroep als fallback,
            # maar de voorkeur gaat uit naar de header.
...
def admin_list_files(endpoint):
    ...
    BASE_DIR = os.path.dirname(os.path.abspath(__file__))
    UPLOAD_FOLDER = os.path.join(BASE_DIR, 'local_storage')
    endpoint_path = os.path.join(UPLOAD_FOLDER, endpoint)
        ...
        if os.path.isdir(client_path):
            for filename in os.listdir(client_path):
                ...
                all_files.append({
                    ...
                    'url': f"/api/{endpoint}/files/{filename}?client_id={client_id}"
                })
    return jsonify(all_files)

@file_bp.route('/admin/files/<endpoint>/<client_id>/<path:filename>', methods=['DELETE'])
def admin_delete_file(endpoint, client_id, filename):
    """
    (ADMIN) Verwijder een bestand.
    """
    BASE_DIR = os.path.dirname(os.path.abspath(__file__))
    UPLOAD_FOLDER = os.path.join(BASE_DIR, 'local_storage')
    file_path = os.path.join(UPLOAD_FOLDER, endpoint, client_id, filename)
